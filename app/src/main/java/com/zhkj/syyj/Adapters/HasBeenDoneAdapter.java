...
public class HasBeenDoneAdapter extends HelperRecyclerViewAdapter<DoneListBean.DataBean> {
    ...
    private List<SecGoodsPriceSBean.DataBean.SpecBean> specList=new ArrayList<>();
    private List<SecGoodsPriceSBean.DataBean.SpecGoodsPriceBean> SpecGoodsPriceList=new ArrayList<>();
    private SecGoodsPriceSBean.DataBean data;
    ...
    protected void HelperBindData(HelperRecyclerViewHolder viewHolder, int position, DoneListBean.DataBean item) {
        ...
        tv_placeOrder.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                getSpecGoodsPrice(item.getGoods_id());
            }
        });
    ...
    public void getSpecGoodsPrice(int goods_id){
        OkGo.<String>get(RequstUrlUtils.URL.getSpecGoodsPrice)
                .params("goods_id",goods_id)
                .execute(new StringCallback() {
                    @Override
                    public void onSuccess(Response<String> response) {
                        SecGoodsPriceSBean secGoodsPriceSBean = new GsonBuilder().create().fromJson(response.body(), SecGoodsPriceSBean.class);
                        if (secGoodsPriceSBean.getCode()==1){
                            data = secGoodsPriceSBean.getData();
                            specList = data.getSpec();
                            SpecGoodsPriceList = data.getSpec_goods_price();
                            key_name="";
                            item_id=0;
                            RedeemBuyNowDialog();
                        }
    ...
    public class TypeAdapter extends BaseAdapter {
        ...
        public View getView(final int position, View convertView, ViewGroup parent) {
            ...
            List<SecGoodsPriceSBean.DataBean.SpecBean.SpecItemBean> spec_item = specList.get(position).getSpec_item();
            LabelsView labelsView = inflate.findViewById(R.id.goods_type_labels);
            labelsView.setMaxLines(0);
            labelsView.setLabels(spec_item, new LabelsView.LabelTextProvider<SecGoodsPriceSBean.DataBean.SpecBean.SpecItemBean>() {
                @Override
                public CharSequence getLabelText(TextView label, int position, SecGoodsPriceSBean.DataBean.SpecBean.SpecItemBean data) {
